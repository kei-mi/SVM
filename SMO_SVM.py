...
class SMO_SVM:
  ...
  def plot_scatter(self):
    if self.dim == 2:
      ...
      x1_min = min(self.X[:,1])
      x1_max = max(self.X[:,1])
      plt.xlim(x0_min-0.1*abs(x0_min), x0_max+0.1*abs(x0_max))
      plt.ylim(x1_min-0.1*abs(x1_min), x1_max+0.1*abs(x1_max))
